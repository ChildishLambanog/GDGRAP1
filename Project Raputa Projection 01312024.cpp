...
int main(void)
{
    ...
    window = glfwCreateWindow(window_width, window_height, "Rafael Ira Rambano Villanueva", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
    ...
    float axis_x = 0, axis_y = 1, axis_z = 0;
                                     //left most, right most, bottom most, top most, z near, z far 
   /* glm::mat4 projectionMatrix = glm::ortho(-2.f, 2.f, -2.f, 2.f, -1.f, 1.0f);*/

    glm::mat4 projectionMatrix = glm::perspective(glm::radians(60.f), window_height / window_width, 0.1f, 100.f);
